...
v6.1 — All bugs fixed, security improved
"""
import asyncio
import logging
import sys
...
def is_valid_telegram_user(user_id) -> bool:
    ...
    if isinstance(user_id, str):
        # Гостевые ID с префиксом guest_ — НЕ валидны для Telegram операций
...
async def create_user_if_not_exists(user_id: int, username: str = None) -> Optional[Dict]:
    ...
    if not user:
        now = int(time.time())
        username_val = (username or '')[:50]  # Ограничиваем длину
...
async def api_get_balance(request: Request) -> Response:
    try:
        ...
        # Валидация: только числовые ID Telegram
...
async def api_game_result(request: Request) -> Response:
    try:
        ...
        # Блокировка для атомарной операции
        lock = await get_balance_lock(user_id)
        ...
        async with aiohttp.ClientSession() as session:
            ...
            try:
                ...
                if result.get("ok"):
                    ...
                    return json_response({"success": False, "error": "CryptoPay API error: " + str(result.get('error', 'unknown'))}, status=500)
...
async def api_withdraw(request: Request) -> Response:
    try:
        ...
        async with lock:
            ...
            # Сбрасываем счётчик игр после вывода
            await sqlite_pool.execute(
...
async def support_receive(message: Message, state: FSMContext):
    ...
    await sqlite_pool.execute(
        ...
        (user_id, msg_text[:500], int(time.time()))  # Ограничиваем длину
    )
    await sqlite_pool.commit()
    ...
    for admin_id in config.ADMIN_IDS:
        try:
            ...
            logger.error(f"Failed to notify admin about support message: {e}")
...
async def admin_clear_db(callback: CallbackQuery):
    ...
    try:
        ...
        # Не удаляем пользователей, только сбрасываем статистику
        await sqlite_pool.execute("UPDATE users SET total_games=0, wins=0, free_spins=0, games_since_withdrawal=0")
        await sqlite_pool.commit()
        
        await callback.message.edit_text(f"✅ База очищена (пользователи сохранены)\n📦 Бэкап: `{backup}`", parse_mode=ParseMode.MARKDOWN, reply_markup=back_keyboard())
...
async def save_mp_room_to_db(room_id: str):
    ...
    # Удаляем старые записи игроков для этой комнаты
...
async def mp_timer_task(room_id: str):
    ...
    if room["status"] == "waiting" and len(room["players"]) >= config.MIN_PLAYERS_MULTIPLAYER: 
        ...
        logger.info(f"🗑 Room {room_id} closed: not enough players")

async def start_mp_game(room_id: str):
    """Запуск игры в мультиплеере"""
    room = mp_rooms.get(room_id)
    ...
    await asyncio.sleep(5)  # Анимация вращения
    ...
    # Очистка комнаты через 3 секунды
...
async def cleanup_old_rooms():
    ...
    while True:
        ...
        for room_id in list(mp_rooms.keys()):
            room = mp_rooms.get(room_id)
            if room and now - room.get("round_start", now) > 600:  # Старше 10 минут
                for uid, pdata in room["players"].items(): 
                    await return_bet(uid, pdata["bet"])
                ...
                try:
                    ...
                    if action == "connect":
                        ...
                        if not is_valid_telegram_user(uid_raw):
                            await ws.send_json({"type": "error", "message": "Invalid user. Please open via Telegram."})
                            continue
                        ...
                        room = mp_rooms.get(room_id)
                        if not room: 
                            await ws.send_json({"type": "error", "message": "Комната завершена"})
                            continue
                        if room["status"] != "waiting": 
                            await ws.send_json({"type": "error", "message": "Игра уже идёт"})
                            continue
                        ...
                        logger.info(f"⬆️ User {user_id} raised bet by {extra}$ in room {room_id}")
...
# Порядок роутеров важен: support → admin → user
...
async def on_shutdown(app):
    ...
    # Возвращаем ставки из активных комнат
...
def create_app() -> web.Application:
    ...
    # API endpoints
    app.router.add_get("/health", lambda r: web.json_response({"status": "ok", "time": int(time.time())}))
